/*
Creates a CloudWatch Log Group that your ECS tasks (Nexus IQ Server) will write logs to.

*/

resource "aws_cloudwatch_log_group" "iq_logs" {
  name              = "/ecs/ref-arch-nexus-iq-server" # standard ECS naming convention
...
/*
🧾 What’s inside the Fluent Bit config
🔹 [SERVICE]
Flush interval, log level, parser files, HTTP metrics endpoint
🔹 [INPUT]
Reads logs from multiple files:
clm-server.log → iq.application
request.log → iq.request
audit.log → iq.audit
policy-violation.log → iq.policy_violation
stderr.log → iq.stderr
Options like Read_from_Head, Refresh_Interval, DB path
🔹 [FILTER]
Adds ECS cluster, task family, region metadata
Adds hostname to all logs
🔹 [OUTPUT]
Sends logs to multiple destinations:
CloudWatch Logs (group: /ecs/ref-arch-nexus-iq-server)
Separate streams for each log type
Local file storage under /var/log/nexus-iq-server/aggregated/...
Optional S3 bucket if enable_log_archive = true

*/
resource "aws_ssm_parameter" "fluent_bit_config" {
  # Namespaced under /ecs/...
  name        = "/ecs/ref-arch-nexus-iq-server/fluent-bit-config"
  description = "Fluent Bit configuration for Nexus IQ Server log parsing"
  type        = "String"
  # Supports longer strings (over 4 KB) [Standard(4KB)]
  tier        = "Advanced"
  value       = <<-EOF
[SERVICE]
    Flush         5
...
/*
This is a Fluent Bit parser configuration, defining how logs are parsed before sending to outputs like CloudWatch, files, or S3.
1️⃣ iq_request parser
[PARSER]
    Name         iq_request
    Format       regex
    Regex        ^(?<client_host>[^ ]*) ...
...
---
Parses request logs (like HTTP requests)
Uses regex to extract fields: client_host, method, path, status, etc.
Converts certain fields to integer (status, bytes, elapsed_time)
Maps log timestamps to time using a custom format
---
*/
